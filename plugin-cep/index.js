...
function buildExecuteScript(actionJson) {
  ...
  return [
    ...
    '      log.push("Track V" + (ti+1) + " => " + tr.clips.numItems + " clips");',
    ...
    '        log.push("  [V" + (ti+1) + " clip" + ci + "] " + cl.name + "  " + cs.toFixed(2) + "s-" + ce.toFixed(2) + "s  match=" + hit);',
    '        $.writeln("[V" + (ti+1) + " clip" + ci + "] " + cl.name + " " + cs.toFixed(2) + "-" + ce.toFixed(2) + "s match=" + hit);',
    '        if (hit) matched.push({ clip: cl, ti: ti });',
    '      }',
    '    }',
    ...
    '        log.push("Inspecting clip: " + cl.name);',
    '',
    '        // Approach 1: walk all components looking for Source Text',
    '        var comps = cl.components;',
    '        log.push("  Components: " + comps.numItems);',
    '        for (var cii=0; cii<comps.numItems; cii++) {',
    '          var comp = comps[cii];',
    '          log.push("  Comp[" + cii + "] displayName=" + comp.displayName);',
    '          $.writeln("  Comp[" + cii + "] " + comp.displayName);',
    '          for (var pi=0; pi<comp.properties.numItems; pi++) {',
    '            var prop = comp.properties[pi];',
    '            var val = ""; try { val = String(prop.getValue()); } catch(pe) { val = "[err:" + pe.message + "]"; }',
    '            log.push("    Prop: " + prop.displayName + " = " + val.substring(0,100));',
    '            $.writeln("    Prop: " + prop.displayName + " = " + val.substring(0,100));',
    '            if (prop.displayName === "Source Text") {',
    '              log.push("    >>> FOUND Source Text");',
    '              if (val.toLowerCase().indexOf(action.find.toLowerCase()) !== -1) {',
    '                prop.setValue(val.replace(new RegExp(action.find, "gi"), action.replace));',
    '                log.push("    >>> REPLACED [" + action.find + "] with [" + action.replace + "]");',
    '                textFound = true;',
    '              } else {',
    '                log.push("    >>> find string [" + action.find + "] NOT in text");',
    '              }',
    '            }',
    '          }',
    '        }',
    '',
    '        // Approach 2: getMGTComponent',
    '        if (!textFound && cl.getMGTComponent) {',
    '          try {',
    '            var mgt = cl.getMGTComponent();',
    '            if (mgt) {',
    '              log.push("  MGT component props: " + mgt.properties.numItems);',
    '              for (var mpi=0; mpi<mgt.properties.numItems; mpi++) {',
    '                var mp = mgt.properties[mpi];',
    '                var mv = ""; try { mv = String(mp.getValue()); } catch(mpe) { mv = "[err]"; }',
    '                log.push("    MGT[" + mpi + "] " + mp.displayName + " = " + mv.substring(0,100));',
    '                $.writeln("    MGT[" + mpi + "] " + mp.displayName + " = " + mv.substring(0,100));',
    '                if (mv.toLowerCase().indexOf(action.find.toLowerCase()) !== -1) {',
    '                  mp.setValue(mv.replace(new RegExp(action.find, "gi"), action.replace));',
    '                  log.push("    >>> MGT REPLACED [" + action.find + "]");',
    '                  textFound = true;',
    '                }',
    '              }',
    '            } else { log.push("  getMGTComponent => null"); }',
    '          } catch(mgte) { log.push("  getMGTComponent error: " + mgte.message); }',
    '        }',
    '',
    '        // Approach 3: projectItem inspection',
    '        if (!textFound && cl.projectItem) {',
    '          try {',
    '            var pi2 = cl.projectItem;',
    '            log.push("  projectItem name=" + pi2.name + " type=" + pi2.type);',
    '            log.push("  mediaPath=" + (pi2.getMediaPath ? pi2.getMediaPath() : "n/a"));',
    '          } catch(pie) { log.push("  projectItem error: " + pie.message); }',
    '        }',
    '      }',
    '      result = textFound ? "OK" : "ERROR: text [" + action.find + "] not found in any matched clip";',
    '    }',
    '',
    '    // ── lumetri_color ────────────────────────────────────────────────',
    '    if (a === "lumetri_color") {',
